...
- **Tolerance resolution (`resolveTol`):** every OOT check runs through `qcDev` → `resolveTol(req,tol)`, which picks the band in priority order: (1) the explicit **Tolerance Range** field, (2) a ± embedded in the requirement callout text (`parseEmbeddedTol`), (3) a **title-block default** by decimal place / angle (`defaultTolBand`). Defaults live in `_fairTolDefaults` (`{p1,p2,p3,ang}`), are editable on the FAR form (`qcSetTolDefault`, manager-only), persist on the record as `tolDefaults`, and are marked with a `*` in the tolerance column. `charRow` computes row status live so changing a default (or opening an older record) reflects immediately. The measurement dialog's tolerance field is **editable for managers** (read-only for operators) and shows which tolerance is being applied.
